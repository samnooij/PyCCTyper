...
class XGBTrain(object):
    ...
    def prune_input(self):
        ...
        if self.undersample > 0:
            print(
                "Undersampling to "
                + str(self.undersample)
                + " repeats for each subtype"
            )
            self.dat = (
                self.dat.groupby("Type")
                .apply(lambda x: x.sample(min(self.undersample, len(x))))
    ...
    def prepare_data(self):
        ...
        X = pd.DataFrame(
            [dict(zip(self.can_kmer, np.zeros(len(self.can_kmer))))]
            + [self.count_kmer(x) for x in self.dat["Seq"]]
        ).fillna(0)
        X = X.iloc[1:]
        ...
        X_train, X_test, y_train, self.y_test = train_test_split(
            X, y, test_size=self.test_size, random_state=self.rnd_seed
        )
    ...
    def train(self):
        ...
        print(
            "\033[92m"
            + "Cross-validating with {} folds:".format(self.nfold)
            + "\033[0m"
        )
        ...
        for max_depth, subsample, colsample_bytree in grid_params:
            print(
                "CV with max_depth={}, subsample={}, colsample_bytree={}".format(
                    max_depth, subsample, colsample_bytree
                )
            )
