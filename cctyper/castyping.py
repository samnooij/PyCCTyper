...
class Typer(object):
    ...
    def type_operon(self, operon):
        ...
        tmp["Interf"] = tmp.apply(
            lambda x: any([i in x["Hmm"] for i in interf_genes]), axis=1
        )
        tmp["Adapt"] = tmp.apply(
    ...
    def cluster_adj(self, data):
        ...
        if self.circular:
            pos_range = max(
                self.genes[self.genes["Contig"] == list(data["Acc"])[0]]["Pos"]
            ) * [0]
        else:
        ...
        pos_range_dilated = ndimage.morphology.binary_closing(
            pos_range_pad, structure=list(np.ones(dist + 1))
        )
    ...
    def typing(self):
        ...
        if self.any_cas:
            ...
            self.hmm_df.loc[:, "operon"] = list(
                chain.from_iterable([x[0] for x in list(operons)])
            )
            ...
            if any(any_circ):
                self.circ_operons = [
                    sorted(i)[0]
                    for (i, v) in zip([x[0] for x in list(operons)], any_circ)
                    if v
                ]
            ...
            single_effector_hmms = self.scores[
                self.scores["Hmm"].isin(list(specifics))
            ].drop("Hmm", axis=1)
            single_effector_hmms[single_effector_hmms < 0] = 0
            self.single_effector = list(
                single_effector_hmms.iloc[
                    :, single_effector_hmms.sum(axis=0).values > 0
                ].columns
            )
    ...
    def write_type(self):
        ...
        if self.any_operon:
            operons_good = self.preddf[
                ~self.preddf["Prediction"].isin(["False", "Ambiguous"])
            ]
            operons_put = self.preddf[
                self.preddf["Prediction"].isin(["False", "Ambiguous"])
            ]
