...
class Controller(object):
    ...
    def __init__(self, args):
        ...
        if self.redo:
            if os.path.exists(self.out + "cas_operons.tab") or os.path.exists(
    ...
    def check_fasta(self):
        ...
        if self.num_headers:
            ...
            self.len_dict = {
                "Contig" + str(key): val for key, val in self.len_dict.items()
            }
            self.seq_dict = {
                "Contig" + str(key): val for key, val in self.seq_dict.items()
            }
