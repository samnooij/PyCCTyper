...
class Map(object):
    ...
    def draw_gene(self, start, end, strand, name, n, z, put):
        ...
        if strand > 0:
            self.im.append(
                draw.Lines(
                    10 + self.scale / 50 * start,
                    self.imheight - (n * 20 * self.scale),
                    10 + self.scale / 50 * end - 3 * self.scale,
                    self.imheight - (n * 20 * self.scale),
                    10 + self.scale / 50 * end,
                    self.imheight - (2.5 * self.scale + n * 20 * self.scale),
                    10 + self.scale / 50 * end - 3 * self.scale,
                    self.imheight - (5 * self.scale + n * 20 * self.scale),
                    10 + self.scale / 50 * start,
                    self.imheight - (5 * self.scale + n * 20 * self.scale),
                    10 + self.scale / 50 * start,
                    self.imheight - (n * 20 * self.scale),
            ...
            self.im.append(
                draw.Lines(
                    10 + self.scale / 50 * start + 3 * self.scale,
                    self.imheight - (n * 20 * self.scale),
                    10 + self.scale / 50 * end,
                    self.imheight - (n * 20 * self.scale),
                    10 + self.scale / 50 * end,
        ...
        if z % 2 == 1:
            self.im.append(
                draw.Text(
                    name,
                    font_size,
                    self.scale / 50 * start + 10,
                    self.imheight - (n * 20 * self.scale - 1 * self.scale),
                    fill="black",
                )
            )
        else:
            self.im.append(
                draw.Text(
                    ...
                    font_size,
                    self.scale / 50 * start + 10,
                    self.imheight - (n * 20 * self.scale + 8 * self.scale),
                    fill="black",
                )
            )
    ...
    def criscas_len(self, cc, cca):
        ...
        for index, row in cc.iterrows():
            ...
            if not (
                row["Operon"] in self.cc_circ_start.keys()
    ...
    def get_longest(self, crisO, casO, criscasO, crisA):
        ...
        if len(casO) > 0:
            ...
            if len(casO_circ) > 0:
                casO_circ["size"] = casO_circ.apply(
                    lambda x: self.len_dict[x["Contig"]], axis=1
                )
                casO_circ_M = max(
                    casO_circ["Start"] + casO_circ["size"] - casO_circ["Start"]
                )
    ...
    def expandCas(
        self, contig, pos, startPos, endPos, seq_size, span_ends, array=False
    ):
        ...
        if span_ends:
            ...
            add_starts = [
                (
                    ...
                    if x[1]
                    else self.expand + 1 + x[0] + seq_size - startPos
                )
                for x in zip(list(add_these["Start"]), which_end)
            ]
            add_ends = [
                (
                    self.expand + 1 + x[0] - startPos
                ...
                for x in zip(list(add_these["End"]), which_end)
            ]
        else:
            add_these = add_these[
                (add_these["Start"] > startPos - self.expand)
        ...
        expand_list = list(
            zip(add_starts, add_ends, list(add_these["Strand"]), names, puts)
        )
    ...
    def expandCris(self, contig, crisprs, startPos, endPos, seq_size, span_ends):
        ...
        if span_ends:
            add_crisp = add_crisp[
                (add_crisp["Start"] > startPos - self.expand)
                | (add_crisp["End"] < endPos + self.expand)
            ]
        else:
            add_crisp = add_crisp[
                (add_crisp["Start"] > startPos - self.expand)
        ...
        if len(add_crisp) > 0:
            ...
            repsCris = [
                list(add_crisp[add_crisp["CRISPR"] == x]["N_repeats"])[0]
                for x in crisp_lst
            ]
            ...
            if span_ends:
                ...
                startsCris = [
                    ...
                    for x in zip(startsCris, which_end)
                ]
                endsCris = [
                    (
                        self.expand + 1 + x[0] - startPos
    ...
    def plot(self):
        ...
        if self.any_operon:
            ...
            try:
                ...
                casAmbiOrph = casAmbiOrph[
                    ~casAmbiOrph["Operon"].isin(self.crispr_cas["Operon"])
                ]
            except:
        ...
        if (not self.noplot) and total > 0:
            ...
            self.interf_genes = set(
                [
                    subitem
                    for subsublist in [
                        item
                        for sublist in self.compl_interf.values()
                        for item in sublist
                    ]
                    for subitem in subsublist
                ]
            )
            self.adapt_genes = set(
                [
                    subitem
                    for subsublist in [
                        item
                        for sublist in self.compl_adapt.values()
                        for item in sublist
                    ]
                    for subitem in subsublist
                ]
            ...
            if not self.nogrid:
                ...
                for x in range(step_size, self.im.width, step_size):
                    ...
                    self.im.append(
                        draw.Text(
                            str(int(x / (self.scale / 50))),
                            22,
                            x - 40,
                            self.imheight - 7 * self.scale,
                            fill="grey",
                        )
                    )
                ...
                for i in list(self.crispr_cas["Operon"]):
                    ...
                    if self.criscaspos[i][3]:
                        startsCas = [
                            (
                                ...
                                if x >= startPos
                                else self.expand + 1 + x + seq_size - startPos
                            )
                            for x in startsCas
                        ]
                        endsCas = [
                    ...
                    cas_list = list(
                        zip(
                            ...
                            strands,
                            nameCas,
                            list((False,) * len(nameCas)),
                        )
                    )

                    # Expand
                    expand_list = self.expandCas(
                        contig,
                        posCas,
                        startPos,
                        endPos,
                    ...
                    cris_list = (
                        list(zip(startsCris, endsCris, nameCris, repsCris))
                        + expand_cris
                    )
                ...
                for i in list(casAmbiOrph["Operon"]):
                    ...
                    cas_list = list(
                        zip(
                            ...
                            strands,
                            casName,
                            list((False,) * len(casName)),
                        )
                    )

                    # Expand
                    expand_list = self.expandCas(
                        contig, pos, startPos, endPos, seq_size, span_ends
                    )
                    cas_list = cas_list + expand_list

                    if self.any_crispr:
                        expand_cris = self.expandCris(
                            contig, [], startPos, endPos, seq_size, span_ends
                        )
                    else:
                        expand_cris = []
