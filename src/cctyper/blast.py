...
class RepeatMatch(object):
    ...
    def clust(self):
        ...
        if len(self.df) > 0:
            ...
            if len(cluster_array) > 0:
                ...
                self.df_array = self.df_cluster[
                    self.df_cluster["Cluster"].isin(cluster_array)
                ]
                self.convert_array()
    ...
    def convert_array(self):
        ...
        for cl in cls:
            ...
            # Get spacers
            spacers = [
                str(self.get_sequence(acc, x[0] + 1, x[1] - 1))
                for x in zip(tmp["Max"][: (len(tmp) - 1)], tmp["Min"][1:])
            ]
        ...
        if len(crisprs) > 0:
            ...
            if hasattr(self, "crisprs"):
                for crisp in crisprs:
                    ...
                    crisp_in_seq = [
                        x for x in self.crisprs if x.sequence == crisp.sequence
                    ]
                    if not any(
                        [
                            x.start <= crisp.end and x.end >= crisp.start
                            for x in crisp_in_seq
                        ]
                    ):
        ...
        if len(self.crisprs) > 0:
            with open(self.out + "crisprs.gff", "w") as fh:
                ...
                for cr in self.crisprs:
                    # Write the parent
                    fh.write(
                        "{}\tCCTyper\trepeat_region\t{}\t{}\t{}\t+\t.\tID={};Note={};Dbxref=SO:0001459;Ontology_term=CRISPR\n".format(
                            cr.sequence,
                            cr.start,
                            cr.end,
                    ...
                    for seq in all_seqs:
                        ...
                        if k % 2:
                            fh.write(
                                "{}\tCCTyper\tdirect_repeat\t{}\t{}\t{}\t+\t.\tID={}_REPEAT{};Parent={};Note={};Dbxref=SO:0001459;Ontology_term=CRISPR\n".format(
                                    cr.sequence,
                                    seq_start,
                                    seq_end,
                                    len(seq),
                        ...
                        if not k % 2:
                            fh.write(
                                "{}\tCCTyper\tbinding_site\t{}\t{}\t{}\t+\t.\tID={}_SPACER{};Parent={};Note={};Dbxref=SO:0001459;Ontology_term=CRISPR\n".format(
                                    cr.sequence,
                                    seq_start,
                                    seq_end,
                                    len(seq),
