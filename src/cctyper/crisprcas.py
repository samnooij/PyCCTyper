        ...
        if self.any_operon and self.any_crispr:
            ...
            for contig in set(cas["Contig"]):
                ...
                for operon in set(cas_sub["Operon"]):
                    ...
                    dists = dist_ll(
                        (
                            int(cas_operon["Start"].iloc[0]),
                            int(cas_operon["End"].iloc[0]),
                        ),
                        zip(crispr_sub["Start"], crispr_sub["End"]),
                        seq_size,
                        circ_op,
                    )
                    ...
                    crispr_operon = crispr_sub[
                        [x <= self.crispr_cas_dist for x in distances]
                    ]

                    # Which CRISPRCas are spanning ends
                    crispr_circ_start = crispr_sub.iloc[
                        [
                            x <= self.crispr_cas_dist and y == "crispr_start"
                            for x, y in dists
                        ],
                        :,
                    ]
                    crispr_circ_end = crispr_sub.iloc[
                        [
                            x <= self.crispr_cas_dist and y == "crispr_end"
                            for x, y in dists
                        ],
                        :,
                    ]
                    ...
                    if len(crispr_operon) > 0:
                        outdict = {
                            ...
                            "Operon_Pos": [
                                list(cas_operon["Start"])[0],
                                list(cas_operon["End"])[0],
                            ],
            ...
            if len(dicts) > 0:
                ...
                self.orphan_crispr = crispr[
                    crispr["CRISPR"].isin(
                        set(crispr["CRISPR"]).difference(
                            set([x for x in crispr_cas["CRISPRs"] for x in x])
                        )
                    )
                ]
                ...
                crispr_near_cas = crispr[
                    crispr["CRISPR"].isin(
                        set([x for x in crispr_cas["CRISPRs"] for x in x])
                    )
                ]
                crispr_near_cas.to_csv(
                    self.out + "crisprs_near_cas.tab", sep="\t", index=False
                )

                pred_lst = []
                for index, row in crispr_cas.iterrows():
                    # Choose nearest CRISPR prediction
                    Prediction_CRISPR = row["Prediction_CRISPRs"][
                        row["Distances"].index(min(row["Distances"]))
                    ]
            ...
            if len(dicts) > 0:
                ...
                if len(crispr_cas_good) > 0:
                    crispr_cas_good.to_csv(
                        self.out + "CRISPR_Cas.tab", sep="\t", index=False
                    )
